...
let y; //전역변수, 값x (undefined)
let z; //전역변수, 값x (undefined)
console.log(x, y, z) //1 undefined undefined(지역값 인식x)
//함수호출
func_z(); //1+undfinde -> NaN == Nat a Number 약자(숫자가 아니라 계산할 수 없음)
//함수준비
function func_z(){
    z = 5; //전역변수z 값5대입위치 = 지역스코프(전역에서 인식못함)
    func_y(); //지역변수 y10 값 인식을 위해 y10을 가지고 있는 함수 호출
    console.log(x+y) //11
}
function func_y(){
    y = 10; //전역변수y 값10대입위치 = 지역스코프(전역에서 인식못함)
...
let snack;
//함수호출
console.log(snack); //undefined (roomA호출 전)
...
function roomA(){
    ...
    console.log(`A방에서 먹는 간식 : ${snack}`); //전역변수 호출(roomA호출해야 가능)
}
function roomB(){//함수 블록스코프 안
    let drink ='콜라';
    console.log(`B방에서 먹는 간식 : ${snack}, ${drink}`); //전역변수 호출(roomB호출해야 가능)
}
//--------------------지역&전역변수 문제3
//지갑에 든 물건 예측하기
//지갑 - 동전, 지갑 - 지폐
...
//함수호출
bank(); //지역변수 값 '지폐'
console.log(`wallet : ${wallet}`)  //전역변수 값 '동전'
//함수준비
function bank(){ 
    let wallet = '지폐'; //지역변수
...
//--------------------매개변수 활용 함수 응용2
//헨젤과 그레텔의 집찾는 재료(매개변수) 찾아주기
// house('돌맹이');
house(); //매개변수의 기본값이 있을 때 호출 괄호는 비워도 됨
house('빵부스러기');
function house(stuff='돌맹이'){
    let story = '';
...
//--------------------커피주문 매개변수 함수
//출력예시 : 손님요청에 따라 다른 메뉴와 잔 수가 출력
//카페라떼 1잔 주문완료
//아메리카노 2잔 주문완료
coffee('카페라떼')
coffee('아메리카노', 2)
function coffee(meun, num=1){
    console.log(`${meun} ${num}잔 주문완료`)
}
...
function cart(name, price, num=1){
    let item = '';
    item += `상품명 :${name}`;
