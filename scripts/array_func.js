...
const color = ['노란색', '파란색', '오렌지색', '초록색', '분홍색', '보라색'];
console.log(color);
console.log(`내가 좋아하는 색상은 ${color[5]},${color[4]} 입니다.`);

// 배열 생성법 2.
// new Array() 내장함수 선언법

let wh = new Array(2); //숫자 2대입(x), 빈배열위치 2자리 삽입(o)
console.log(wh);
...
function weatherFunc(today, tomorrw){
    let msg = `오늘은 ${today}, 내일은 ${tomorrw} 소식이 있습니다.`;
    console.log(msg);
}

//----------------------영화 좌석 예매 배열+함수
//일반 2명 좌석 선택 기준
//일반 1명당 표 가격 10,000원 => 2명 20,000원
let cgvSeat = new Array(2); //빈 좌석 2개 준비
const price = 10000; //일반 1명 만원
cgvSeat[0] = 'C1';
cgvSeat[1] = 'C2';
//함수 호출
...
dan99(2); //()안에 생성된 매개변수 자리에 대입해야하는 값 입력위치
dan99(3);
dan99(4);
dan99(5);
dan99(6);
