...
## Swiper 작성 준비
0. `head` 태그 안 `title` 아래 swiper 사용을 위한 CDN연결
...
1. `html` 파일 내에서 슬라이드 삽입위치에 다음과 같이 태그작성
...
2. 1번에서 생성한 태그에 대상구분용의 별도 클래스를 따로 생성
    `div.swiper.개별클래스명`
...
* `loop` : true, false(반복안함)
* `direction` : 'vertical', 'horizontal' 슬라이드 방향 (horizontal(수평)이 기본값)
    * `direction:'vertical',`
    * 제자리 나타나기 / 사라지기 할 때는 direction 적용해제ㄴ
* `effect` : fade, cube, flip 
* `mousewheel`:true 마우스휠 가능 옵션
* 자동재생
    * autoplay:{
        selay:1000, **1000 == 1초 (1초씩 슬라이드 넘어감)**
        pauseOnMouseEnter:true, **마우스 올라오면 일시정지**
        disableOnInteraction:true, **마우스가 상호적용 후에 멈출건지, 움직일건지(false움직임이 기본값)**
    }
## Swiper 선택요소(페이지번호, 이전다음버튼, 스크롤바) 주의사항
* 기본 작성 위치는 `swiper-wrapper`의 다음 형제 위치
    * `swiper`의 첫번째 자식(swiper-wapper) 두번째자식(선택요소들)
...
### 기존 Swiper-slide와 겹치는 디자인을 하고싶을 때
* 태그 위치 `div.swiper` 두번째 자식 위치로 생성
### 기존 Swiper-slide 외부 바깥쪽으로 안 겹치는 디자인을 하고싶을 때
* 태그 위치 `div.swiper` 다음 형제 위치로 생성
* `div.swiper`와 생성된 선택태그를 묶어주는 추가 태그 생성(디자인에 따라 다름)
### 교차 여부에 따라 태그 위치를 다르게 해야하는 이유
* `div.swiper`에 기본 CSS로 `overflow:hidden` 있으므로 그 값을 `visible`로 바꾸면 선택 요소 뿐 아닌 `swiper-slide`들도 모두 보이기 때문에 원하는 레이아웃을 만들 수 없음.
## Swiper 안에 추가 Swiper 생성 시 안쪽 Swiper 마우스드래그 안 될 때 해결방법
...
1. `renderFraction:function(){}`
    ...
    * `const 변수명 = new Swiper(적용대상,{`
        `pagination:{`
            `el:'적용대상'`
            `type:'fraction'`
            `renderFraction:funcktion(curren, total){`
